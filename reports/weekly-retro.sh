...
# Creates weekly progress retrospective and next week focus based on Covey analysis

# Auto-load configuration and logging
source "$(dirname "$0")/../utils/auto-config.sh"
source "$(dirname "$0")/../utils/logger.sh"

log_start "Weekly retrospective analysis"

# Skip if it's the 1st of the month (full analysis day)
if [ $(date +%d) -eq 01 ]; then
    log_info "Skipping weekly review - monthly comprehensive analysis runs today"
    exit 0
fi
...
WEEKLY_RETRO_FILE="$VAULT_DIR/Weekly Retro.md"
ARCHIVED_DIR="$VAULT_DIR/Archived"

if [ -f "$WEEKLY_RETRO_FILE" ]; then
    mkdir -p "$ARCHIVED_DIR"
    mv "$WEEKLY_RETRO_FILE" "$ARCHIVED_DIR/Weekly Retro-${TIMESTAMP}.md"
    log_info "Archived previous weekly retro to $ARCHIVED_DIR/Weekly Retro-${TIMESTAMP}.md"
fi
...
if [ -f "$CLAUDE_OUTPUT" ] && [ -s "$CLAUDE_OUTPUT" ]; then
    ...
    if [ "$QUESTION_COUNT" -gt 0 ]; then
        ...
        echo ""
        echo "ℹ️  Review these insights before continuing with weekly retrospective"
        read -p "Press Enter to continue with weekly review generation..."
...
STEP 2: GREG MCKEOWN ESSENTIALIST WEEKLY RETROSPECTIVE

Channel Greg McKeown's Essentialism approach to cut through the noise and identify what truly mattered this week.

Read the Greg McKeown prompt at: $PROMPTS_DIR/greg-mckeown-essentialist.md

Create 'Weekly Retro.md' in $VAULT_DIR focusing on the VITAL FEW insights:

**ESSENTIAL RETROSPECTIVE QUESTIONS:**
1. What was the ONE thing that made the biggest difference this week?
2. What was the single biggest obstacle that held back progress?
3. Which activities were merely "good" vs truly "great" for your mission?
4. What can you eliminate next week to make space for what's essential?

**MCKEOWN'S 90% RULE APPLIED TO THE WEEK:**
- Review Date: ${DATE_READABLE} | Week of: ${WEEK_OF}
- Task Completion: ${COMPLETION_RATE}% (${CHECKBOX_COMPLETED}/${CHECKBOX_TOTAL})
- Biography Responses: $QUESTION_COUNT

**ESSENTIAL-ONLY SECTIONS:**

### The Vital Few Wins
- Maximum 3 outcomes that truly moved the needle toward your mission
- Eliminate the "busy work" and "good" accomplishments - focus only on what was essential

### The One Critical Bottleneck  
- The single most important obstacle preventing breakthrough
- What is the 20% that's causing 80% of your frustration or stagnation?

### Elimination Opportunities
- What activities/commitments should be eliminated to create space for the essential?
- What "good" things are preventing "great" things from happening?

**ESSENTIALISM VOICE:** Write in Greg McKeown's clear, decisive style that cuts through complexity to reveal what truly matters. Less analysis, more clarity. Focus on disciplined choices over busy activity.

Keep it ruthlessly concise - eliminate everything except the insights that will make the biggest difference next week.
EOF

# Continue with weekly review generation
log_info "Generating weekly review with continued context"
"$SCRIPTS_DIR/utils/claude-wrapper.sh" --continue < /tmp/weekly_flow_review.txt

# Clean up temp file
rm -f /tmp/weekly_flow_review.txt

# Trigger follow-up activities if the retro was generated successfully
if [ -f "$WEEKLY_RETRO_FILE" ]; then
    log_info "Weekly retro generated successfully, triggering follow-up activities"
